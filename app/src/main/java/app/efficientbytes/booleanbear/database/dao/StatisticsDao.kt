...
interface StatisticsDao {
    ...
    @Query("UPDATE user_screen_timing SET closed = :closeTime WHERE date = :currentDate AND closed = -1 AND rowId = (SELECT MAX(rowId) FROM user_screen_timing WHERE date = :currentDate)")
    ...
    suspend fun getTotalScreenTimePerDayBasisForAllDay(): List<ScreenTimingPerDay>
    ...
    suspend fun deleteScreenTimingForAllDay()
